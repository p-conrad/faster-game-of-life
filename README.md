...
a trade-off. A common choice is to limit the grid size to a chosen number and consider any cells
...
2. A *data structure* suitable for keeping the two-dimensional field's state in our chosen type,
...
1. In our first version, we use `bool` for storing a cell's state. As a cell can only ever be in one
   of two states (alive/dead), this seems the natural choice. This will change later.
2. For storing large amounts of data, an array is probably the first structure to come to mind.
   ...
   `width*height` length, storing the number of rows and columns separately. For a given cell in the
   ...
   (By the way: Why use a `std::vector` if the size of our field isn't going to change beyond
   initial allocation? After all, the overhead of using a dynamic container type seems wasteful if
   we're not going to need it. The answer is simple: *Because it doesn't matter.* Internally,
   indexing a `std::vector` is exactly the same as indexing a raw pointer or a `std::array`. You can
   verify this by looking at the resulting assembly. So it's mostly a matter of taste.)
...
4. Counting the neighbours and setting each cell's next state is done using the simple approach,
   the logic being split up into multiple methods:
   ```cpp
   ...
   value from the result. This saves us from having to write each cell access separately. (In case
   you're wondering: The GCC compiler is actually smart enough to figure this out when using the
   highest optimisation level, emitting the same assembly as if the additions were written out by
   hand.)
   Getting each cell's state is handled by the `getElementAt` method, which also handles
   the wrapping around the field's edges for us.
5. For performance measurements, we make use of [OpenMP](https://www.openmp.org/), a parallel
   programming specification and library which we're going use again at later optimisation stages.
   ...
   then written out into a CSV file along with the GPS value (which is just `1.0 / avg`).
   All that functionality is kept in another class, `FieldBenchmark`, which also handles opening and
   finalising the output file, and printing a simple progress bar. The resulting CSV file can be
   opened in a data analysis framework of your choice, but if we just need the average GPS value of
   the entire benchmark, AWK is also sufficient:
   ```
...
6. Graphics output is quite simple. All we need to do is draw a bunch of colored boxes on a black
   background – no bells and whistles. We choose the FLTK framework to avoid having to pull in an
   unnecessarily complex UI library. Though a bit arcane by today's standards, it gets the job done
   nicely in as little as 66 lines of code. As a little addition, pressing the space bar allows us
   to start/stop the game anytime.
...
There is one important thing to note here: While we consider this version our base (or "naive")
version, it isn't actually as bad as it could potentially be. In fact, it already has a few (albeit
not perfect) basic optimisations which would be common sense for developers with a basic knowledge
of optimisation: For starters, it's written in C++, a compiled language with lots of low-level
control and a smart compiler. That alone gains us a speed boost of about 25x if compared to Python.
What's more, the contents of the field are stored in *one* contiguous array, giving us good *data
locality*, resulting in a cache-friendly behaviour. This is not a given – an overzealous junior
developer may be tempted to employ disadvantageous "best practices" and define something like
`BoolCell : AbstractCell<bool>`, with data encapsulation and accessor functions to read or modify
the state, then storing that in nested vectors. This would result in worse data locality, also
adding the overhead of calling instance methods. And if they were to include more members (e.g., x/y
coordinates), or if vital methods were virtual in the base class, or if each cell were even
allocated individually, then all bets would be off.
...
**`std::vector<bool>` is not quite what you expect it to be**. While it would likely be obvious to
more experienced C++ programmers, it's not something one would expect naturally without reading the
[reference on this topic](https://en.cppreference.com/w/cpp/container/vector_bool.html). But let's
conduct a little experiment to see what happens. Here are two (arguably contrived) functions
returning an element in a vector at a given index:
...
given offset in the vector, it involves a substantial amount of bit manipulation. Here is the truth:
**`std::vector<bool>` is a special case** (actually, a controversial one). Instead of storing each
element at its own memory location just like all the other vector types, it packs them together such
that each value takes only one bit of storage. While beneficial if you were handling an enormous
amount of boolean values and care about memory, this is also detrimental to performance.
...
handle certain types a little better than others, which can easily make a speed difference of 10-20%.
...
(horribly inconsistently named) `uint_fast8_t` and be guaranteed to get the fastest type available
on our system. All that is left is to change our logic a bit: `true` becomes 1, `false` becomes 0.
...
Now would be the time to open the profiler to see where the potential for further optimisation lies.
This is an important point which needs to be stressed: *Don't make optimisations based on mere
assumptions.* Take a look which parts of your program run the slowest, *then* try to optimise these.
You could easily end up optimising the wrong parts of your projects, wasting time while making your
code much harder to read or reason about.

With that said, here is what you would see in the profiler: *62% of all execution time is spent in
the `getElementAt` method*. Why is that? Well, for each call, our method has to check if one of
the indices is beyond one of the four edges of our field, and adjust accordingly. That's *four* ifs
for each single call. Given that we are counting eight neighbours for each of our 250.000 cells in
the field, that's *8.000.000* conditions checked for each generation. How wasteful!
...
and copy the contents into the corresponding padding on the other side, which costs us almost
nothing. Now we can eliminate all the conditionals from our `getElementAt` method – if we are at one
of the edges, we simply read the value from the padding. (By the way: This idea was originally taken
...
state, which would in turn call the overloaded index operator of our `SimpleMatrix` class. Oh, and
did I mention that `getElementAt` is itself called by `neighborCount`, which is called by
`nextCellState`, which is then called by `nextGeneration`?
...
slowing down the computation. That's why we do away with some of them now: Specifically, we throw
away our `SimpleMatrix` completely, keeping our vectors in `GameField` directly. For faster access,
we also define some new inlined "accessor" functions, and change our code accordingly:
```cpp
...
Now we're getting to our most significant optimisation so far. But before dive in, I would like to
make it clear that I do not claim any ownership of the idea: It was originally described in the book
...
does not exist (maybe you can `import crystalball` in Python, though). But we can get close!

You see, since we are using unsigned integer values for keeping our cell state, we have some free
bits left in each cell. We could put them to good use and change the encoding a little:
...
1. We can stop counting each cell's neighbours altogether, extracting the number directly instead.
   In lieu of counting, we propagate *only* when a cell's state has changed. This is much less work
...
2. At any given point in time, the larger proportion of our field will just be empty space. A dead
   ...
   allows us to identify these cases immediately: We can skip any cell with a value of zero
   altogether!
...
That's an improvement of 475%! It highlights the importance of sometimes taking a step back and
...
version helps with that quite efficiently.
...
distributing the work on multiple cores. Using OpenMP, the solution is quite easy:
```cpp
#pragma omp parallel
  {
...
sharing data. It can significantly reduce the efficiency of parallel execution by increasing the
...
Now we're going pretty fast! Although, to be honest, an increase of 285% seems a bit underwhelming
